...
1. **Georeference** — upload your map scan and align it on top of a live world map so geographic coordinates are established.
2. **Draw** — manually trace features using interactive drawing tools.
3. **Trace** — let the app automatically detect and vectorise dark lines from the overlay using in-browser bitmap tracing, then review and export the result.
...
| 📍 **Georeferencing**           | Drag, scale (per-axis), rotate, skew and adjust opacity of the image overlay to align it with the background map (inspired by QGIS Georeferencer) |
| 📌 **Pin to map**               | Locks the image to geographic coordinates as a MapLibre `ImageSource`; image then moves with the map                                              |
| ✏️ **Drawing tools**            | Draw points, lines and polygons interactively using `@mapbox/mapbox-gl-draw`                                                                      |
| 📂 **GeoJSON import / export**  | Load an existing `.geojson` file or download your work                                                                                            |
| 🔍 **Bitmap tracing**           | Automatically trace dark lines from the overlay into GeoJSON features; configurable threshold, blur, and simplification                           |
| ↩ **Undo / redo**              | 30-step history; Ctrl+Z / Ctrl+Y                                                                                                                  |
| 💾 **Auto-save**                | Full application state (including image as base64) persisted to `localStorage`                                                                    |
| 📴 **PWA / offline**            | Installable as a Progressive Web App; tile requests cached by Service Worker                                                                      |
| 🌐 **Configurable tile source** | OpenFreeMap (default), OSM raster, any MapLibre style URL, XYZ raster template, or PMTiles (vector or raster) for offline / self-hosted scenarios |
| 🔭 **Military scale presets**   | One-click zoom to 1:25k, 1:50k, 1:100k, 1:250k, 1:500k, 1:1M                                                                                      |
...
| State management | [meiosis-setup](https://meiosis.js.org/) v6                        | Functional stream-based state (mergerino object patches); no magic |
| Map engine       | [MapLibre GL JS](https://maplibre.org/) v5                         | Open-source, WebGL-accelerated, vector + raster + custom sources   |
| Drawing          | [@mapbox/mapbox-gl-draw](https://github.com/mapbox/mapbox-gl-draw) | Mature interactive drawing for MapLibre-compatible maps            |
| Bitmap tracing   | [imagetracerjs](https://github.com/jankovicsandras/imagetracerjs)  | Pure-JS SVG tracer; no WASM dependency, works offline              |
| Geometry         | [@turf/turf](https://turfjs.org/)                                  | Douglas-Peucker simplification of traced paths                     |
| Offline tiles    | [pmtiles](https://github.com/protomaps/PMTiles)                    | Single-file tile archives; loaded via MapLibre custom protocol     |
| Build            | [Vite](https://vitejs.dev/) v6                                     | Fast dev server + Rollup bundler with code splitting               |
| PWA              | [vite-plugin-pwa](https://vite-pwa-org.netlify.app/) / Workbox     | Service Worker generation, tile caching                            |
...
└── src/
    ├── app.ts                      Mount point: wires cells → Mithril root
    ├── declarations.d.ts           Module declaration for imagetracerjs
    ├── assets/
    │   └── style.css               Dark theme, layout, responsive breakpoints
    ├── state/
    │   ├── types.ts                AppState interface and all sub-types
    │   ├── index.ts                meiosisSetup cells, localStorage persistence, exports
    │   └── history.ts              Undo/redo stack (max 30 snapshots)
    ├── components/
    │   ├── App.ts                  Root layout; drag-and-drop, paste, keyboard shortcuts
    │   ├── Toolbar.ts              Mode switcher, military scale buttons, undo/redo, settings toggle
    │   ├── MapView.ts              MapLibre map; PMTiles protocol registration; tile source watcher
    │   ├── ImageOverlay.ts         DOM overlay image with drag/scale/rotate/skew handles; Pin action
    │   ├── DrawPanel.ts            mapbox-gl-draw integration; feature list; import/export
    │   ├── TracePanel.ts           Trace settings UI; run/preview/commit workflow
    │   └── SettingsPanel.ts        Tile source selector (preset + custom)
    └── services/
        ├── mapHelpers.ts           buildMapStyle() — converts TileSourceConfig to MapLibre style
        ├── geojsonExport.ts        downloadGeoJSON(), parseGeoJSON()
        └── imageTracer.ts          Canvas preprocessing → imagetracerjs → SVG → GeoJSON
```
...
interface AppState {
  ...
  ui: {
    mode: 'georeference' | 'draw' | 'trace';
    drawMode: 'none' | 'point' | 'line' | 'polygon' | 'select';
    ...
    traceSettings: {
      colorMode: 'black' | 'color';
      threshold: number;         // 0–255 grayscale cutoff
      simplification: number;    // Turf Douglas-Peucker tolerance
      blurRadius: number;        // px blur before threshold
    };
  };
}
...
State is managed by **meiosis-setup v6** which wraps it in a stream of "cells". Each cell exposes `.state` (read) and `.update(patch)` (write). Object patches are deep-merged by mergerino. Every state change is persisted to `localStorage` and pushed onto the undo history stack.
...
cell.update(patch)
    │
    ├──► meiosis stream emits new cell
    │         │
    │         ├──► saveState()        (localStorage)
    │         ├──► pushHistory()      (undo stack)
    │         └──► m.redraw()         (Mithril VDOM diff → DOM patch)
    │
    └──► MapLibre side-effects (setStyle, easeTo, updateImage …)
         triggered by cell.states.map() subscriptions inside MapView
```

### Georeferencing model

While positioning, the image is a **fixed DOM element** (`position: absolute`, CSS `transform`) layered over the map canvas. The map pans/zooms freely underneath. When the user clicks **Pin to map**:

1. The four corners of the image DOM element are read via `getBoundingClientRect()`.
2. Each pixel position is converted to `[lng, lat]` using `map.unproject()`.
3. A MapLibre [`ImageSource`](https://maplibre.org/maplibre-gl-js/docs/API/classes/ImageSource/) is created with those four coordinates, and a `RasterLayer` renders it — the image now moves with the map.
4. The `geoCorners` array is stored in state for use by the tracer.
...
Draw to offscreen <canvas>
      │
      ▼  optional blur (ctx.filter)
Apply grayscale + threshold  ← pixel < threshold → black, else white
      │
      ▼
imagetracerjs.imageDataToSVG()  ← pure-JS bitmap → SVG string
      │
      ▼
DOMParser: extract all <path d="…"> elements
      │
      ▼
parseSvgPathToSegments()  ← tokenise M/L/Z commands → pixel coordinate arrays
      │
      ▼
pixelToGeo() bilinear interpolation  ← maps pixel [x,y] → [lng,lat]
  using the four pinned geoCorners as control points
      │
      ▼
Classify as LineString (open) or Polygon (closed path)
      │
      ▼
@turf/turf simplify()  ← Douglas-Peucker with configurable tolerance
      │
      ▼
GeoJSON FeatureCollection  → preview UI → user commits to drawing layer
```
...
| **XYZ Raster**            | Any standard tile server (`https://…/{z}/{x}/{y}.png`).                                                  |
| **PMTiles – Vector**      | A single `.pmtiles` file (local file or URL) containing vector tiles. Ideal for fully offline operation. |
| **PMTiles – Raster**      | A single `.pmtiles` file containing raster tiles.                                                        |

For PMTiles, you can either enter a URL or pick a local file from disk — the file is opened in-browser via the [PMTiles protocol](https://github.com/protomaps/PMTiles) registered with MapLibre.

---

## Military Scale Presets

Quick-zoom buttons in the toolbar snap to standard NATO/military map scales:

| Button | Scale         | Approx. MapLibre zoom |
| ------ | ------------- | --------------------- |
| 1:25k  | 1 : 25 000    | 14                    |
| 1:50k  | 1 : 50 000    | 13                    |
| 1:100k | 1 : 100 000   | 12                    |
...
- **File picker** — click **📷 Upload image** at the bottom of the screen.
- **Drag & drop** — drag any image file from your file manager onto the browser window; a drop zone overlay confirms the target.
- **Clipboard paste** — copy an image (e.g. a screenshot) and press **Ctrl+V** / **Cmd+V**.
...
- **Point**, **LineString**, **Polygon** tools
- **Select** tool — click a feature to select; drag to move vertices
- **Trash** — delete selected features

The left sidebar shows all drawn features. Features can be renamed (via the properties panel) and individually deleted. Use **📂 Import** to load an existing GeoJSON, and **💾 Export** to download the current FeatureCollection.

### Step 4 — Trace (automatic)
...
| **Threshold** (10–245)        | Grayscale cutoff — pixels darker than this are treated as lines. Lower = more lines captured; higher = only the darkest ink. |
| **Blur radius** (0–5 px)      | Applied before thresholding. Reduces noise and fills small gaps.                                                             |
| **Simplification** (0–0.001°) | Douglas-Peucker tolerance. Higher = fewer vertices, smoother curves.                                                         |

Click **🔍 Run trace**. A preview shows the number of features detected. Review them, then:

- **✅ Add to GeoJSON** — merges traced features into the drawing layer.
- **💾 Export preview** — downloads only the traced result without adding to the main layer.
- **✕ Discard** — throws away the preview.

You can re-run with different settings as many times as needed.
...
| ------------------------ | ----------- | -------------------------------- |
| `maplibre-….js`          | ~283 kB     | MapLibre GL (largest dependency) |
| `index-….js`             | ~26 kB      | App code                         |
| `draw-….js`              | ~18 kB      | mapbox-gl-draw                   |
| `imagetracer_….js`       | ~6 kB       | imagetracerjs (async chunk)      |
| `turf-….js`              | ~2 kB       | @turf/turf subset                |
| `sw.js` + `workbox-….js` | —           | Service Worker                   |

### Adding a new tile source preset

1. Add a new value to the `TilePreset` union in `src/state/types.ts`.
2. Handle it in `buildMapStyle()` in `src/services/mapHelpers.ts`.
3. Add an `<option>` entry in `src/components/SettingsPanel.ts`.

---

## Deployment (GitHub Pages)

The repository includes `.github/workflows/deploy.yml`. It builds and deploys automatically on every push to `main`.

**Setup:**

1. Go to **Settings → Pages** in your GitHub repository.
2. Set **Source** to **GitHub Actions**.
3. Push to `main` — the site is live at `https://<username>.github.io/image2geojson/`.
